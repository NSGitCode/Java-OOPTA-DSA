...
public class AnimalMonitor 
{
    ...
    {
		/*
		 * for(Sighting record : sightings) { System.out.println(record.getDetails()); }
		 */
    ...
    public void printSightingsOf(String animal)
    {
       /* for(Sighting record : sightings) {
        ...
        sightings.stream()
	      .filter(p -> p.getAnimal().equals(animal))
	      .forEach(p -> System.out.println(p.getDetails()));
    }
    ...
    {
		/*
		 * for(Sighting record : sightings) { if(record.getSpotter() == spotter) {
		 * System.out.println(record.getDetails()); } }
		 */
        
        sightings.stream()
	             .filter(p -> p.getSpotter() == spotter)
	             .map(p -> p.getDetails())
    ...
    {
		/*
		 * for(String animal : animalNames) { if(getCount(animal) <= dangerThreshold) {
    ...
    {
		/*
		 * int total = 0; for(Sighting sighting : sightings) {
		 * if(animal.equals(sighting.getAnimal())) { total = total +
		 * sighting.getCount(); } } return total;
		 */
        
        int total =
				sightings.stream()
				         .filter(p -> animal.equals(p.getAnimal()))
				         .map( p -> p.getCount())
				         .reduce(0, (count, sighting) -> count + sighting);
				          System.out.println("Total sightings: " + total);
				      return total;
    }
